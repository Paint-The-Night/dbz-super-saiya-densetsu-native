...
The bank-3 actor helper XBA at `$03:9755` is now native and differentially verified, adding 1 site and 1 byte. The following actor-helper setup instructions at `$03:9756–$03:9760` (LDA, four DP stores, and XBA) are also native and differentially verified, adding 6 sites and 11 bytes. The width transition at `$03:9761–$03:9765` (REP, 16-bit ASL/TAX, and SEP) is now native and differentially verified, adding 4 sites and 6 bytes. The indexed data setup at `$03:9767–$03:9778` (two long,X loads, four DP stores, two byte shifts, and INX) is now native and differentially verified, adding 9 sites and 19 bytes. The helper epilogue at `$03:977A–$03:977B` (`PLX` and `RTL`) is now native and differentially verified, adding 2 sites and 2 bytes. The bank-$00 actor-type helper at `$00:8AE5–$00:8AEF` (`STZ`, `JSL`, and `RTL`) is now native and differentially verified, adding 4 sites and 11 bytes.
Its immediate mask at `$03:9753` is now native and differentially verified, adding 1 site and 2 bytes.
...
The bank-2 `$D37D–D38A` actor gate is native across its five assigned sites,
...
The bank-0 `$9ACD–9AE2` scene-status prefix is native across all ten assigned
instructions, including helper dispatch, status masking, DMA clear, branch, and RTL.
Differential tests add 10 sites and 22 ROM bytes.
...
The bank-3 `$F561–F581` actor gate is native across all 14 assigned
...
The bank-3 `$F5CB–F5F5` actor gate is native across all 18 assigned
instructions, including alternate phase thresholds, both helper dispatches,
and RTL. Differential tests add 18 sites and 43 ROM bytes.

The bank-3 `$F5A3–F5CA` actor fallback selector is native across all 18
assigned instructions, including its threshold gates, marker publication, and
RTL. Differential tests add 18 sites and 40 ROM bytes.

The bank-1 `$8587–859B` scene status gate is native across all 13 assigned
instructions, including its status nibble checks, carry exits, and three RTL
paths. Differential tests add 13 sites and 21 ROM bytes.
