...
bool dbz_native_step(Cpu *c, DbzExecution *x) {
  ...
  bool done = false;
  if(bank == 1) {
    if(c->pc >= 0x8092 && c->pc <= 0x80a1) {
      done = actor_index_8092_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0x8241 && c->pc <= 0x824f) {
      done = actor_index_8241_step(c, c->pc); x->display_control_steps += done;
    } else if((c->pc >= 0xba10 && c->pc <= 0xba1c) || (c->pc >= 0xba1d && c->pc <= 0xba29)) {
      done = actor_flag_ba10_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xce57 && c->pc <= 0xce6c) {
      done = actor_status_ce57_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xce96 && c->pc <= 0xceab) {
      done = actor_status_ce96_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xceac && c->pc <= 0xceb8) {
      done = actor_status_ceac_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xccef && c->pc <= 0xcd01) {
      done = actor_status_ccef_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xbbf7 && c->pc <= 0xbc03) {
      done = actor_index_bbf7_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xc91f && c->pc <= 0xc959) {
    ...
    } else if(c->pc >= 0xc8ab && c->pc <= 0xc8c7) {
      done = actor_table_c8ab_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xc8e5 && c->pc <= 0xc901) {
      done = actor_table_c8e5_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xc902 && c->pc <= 0xc91e) {
      done = actor_table_c902_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xc236 && c->pc <= 0xc256) {
      done = actor_copy_c236_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd3c0 && c->pc <= 0xd3dc) {
      done = actor_pointer_d3c0_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd1dc && c->pc <= 0xd201) {
      done = actor_marker_d1dc_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0x863a && c->pc <= 0x8650) {
      done = actor_pointer_863a_step(c, c->pc); x->display_control_steps += done;
    ...
    } else if(c->pc >= 0xcf3a && c->pc <= 0xcfa0) {
      done = text_script_cf3a_step(c, c->pc); x->display_control_steps += done;
    ...
    } else if(c->pc >= 0xa4eb && c->pc <= 0xa4f2) {
  ...
  if(bank == 3) {
    if(c->pc == 0x952e || c->pc == 0x9530 || c->pc == 0x9534 || c->pc == 0x9535 || c->pc == 0x9538 || c->pc == 0x953a || c->pc == 0x953d) {
      done = scene_actor_dispatch_952e_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc == 0x9752 || c->pc == 0x9753 || c->pc == 0x9755 || c->pc == 0x9756 || c->pc == 0x9758 || c->pc == 0x975a || c->pc == 0x975c || c->pc == 0x975e || c->pc == 0x9760 || c->pc == 0x9761 || c->pc == 0x9763 || c->pc == 0x9764 || c->pc == 0x9765 || c->pc == 0x9767 || c->pc == 0x976b || c->pc == 0x976d || c->pc == 0x976e || c->pc == 0x9770 || c->pc == 0x9771 || c->pc == 0x9775 || c->pc == 0x9777 || c->pc == 0x9778 || c->pc == 0x977a || c->pc == 0x977b) {
    ...
    if(c->pc >= 0x958a && c->pc <= 0x9599) {
    ...
    } else if(c->pc == 0xf246 || c->pc == 0xf39e || c->pc == 0xf39f || c->pc == 0xf3a0 || c->pc == 0xf4ad || c->pc == 0xf560 || c->pc == 0xf582 || c->pc == 0xf5a2) {
      done = scene_return_bank3_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xf375 && c->pc <= 0xf39d) {
      done = scene_gate_f375_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc == 0xe8aa || c->pc == 0xe8ad) {
      done = scene_actor_e8aa_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xe757 && c->pc <= 0xe768) {
      done = copy_span_e757_step(c, c->pc); x->display_control_steps += done;
    } else if((c->pc >= 0xe769 && c->pc <= 0xe77a) || (c->pc >= 0xe77b && c->pc <= 0xe78c)) {
      done = copy_span_e769_e77b_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xe655 && c->pc <= 0xe66a) {
      done = copy_span_e655_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xa3a8 && c->pc <= 0xa3b8) {
      done = actor_gate_a3a8_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xf7de && c->pc <= 0xf7e6) {
      done = actor_phase_f7de_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xf561 && c->pc <= 0xf581) {
      ...
      done = actor_setup_f5f6_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0x953e && c->pc <= 0x954b) {
      done = actor_prefix_953e_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xb7cb && c->pc <= 0xb7d6) {
      done = actor_scratch_b7cb_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xb7f4 && c->pc <= 0xb7ff) {
      done = actor_scratch_b7f4_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xc232 && c->pc <= 0xc24a) {
      done = actor_seed_c232_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xe6f0 && c->pc <= 0xe70d) {
      done = actor_fill_e6f0_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xf2dd && c->pc <= 0xf2ff) {
      done = actor_bounds_f2dd_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0x9b7f && c->pc <= 0x9b9a) {
      done = actor_selector_9b7f_step(c, c->pc); x->display_control_steps += done;
    }
    return done;
  }
  ...
  if(bank == 5) {
    if(c->pc >= 0x9626 && c->pc <= 0x96d6) {
      ...
      done = actor_op_cf08_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xcf60 && c->pc <= 0xcfb0) {
      done = actor_op_cf60_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xcfb1 && c->pc <= 0xcfb8) {
      done = actor_op_cfb1_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xcfb9 && c->pc <= 0xd037) {
      done = actor_op_cfb9_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd038 && c->pc <= 0xd059) {
      done = actor_op_d038_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd05a && c->pc <= 0xd06c) {
      done = actor_op_d05a_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd06d && c->pc <= 0xd094) {
      done = actor_op_d06d_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd095 && c->pc <= 0xd0b9) {
      done = actor_op_d095_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd0ba && c->pc <= 0xd117) {
      done = actor_op_d0ba_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd118 && c->pc <= 0xd13d) {
      done = actor_op_d118_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd13e && c->pc <= 0xd1d5) {
      done = actor_op_d13e_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd1d6 && c->pc <= 0xd1e0) {
      done = actor_op_d1d6_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd1e1 && c->pc <= 0xd1f3) {
      done = actor_op_d1e1_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd1f4 && c->pc <= 0xd25f) {
      done = actor_op_d1f4_step(c, c->pc); x->display_control_steps += done;
    } else if(c->pc >= 0xd260 && c->pc <= 0xd273) {
      done = actor_op_d260_step(c, c->pc); x->display_control_steps += done;
  ...
  if(bank == 4) {
    ...
    } else if(c->pc >= 0x8e4a && c->pc <= 0x8e5e) {
      done = actor_counter_8e4a_step(c, c->pc); x->display_control_steps += done;
  ...
  } else if(c->pc == 0x9535 || c->pc == 0x9538 || c->pc == 0x953a || c->pc == 0x953c || c->pc == 0x953e || c->pc == 0x9540 || c->pc == 0x9543 || c->pc == 0x9544 || c->pc == 0x9546 || c->pc == 0x954c || c->pc == 0x954e || c->pc == 0x9550 || c->pc == 0x9554 || c->pc == 0x9556 || c->pc == 0x9559 || c->pc == 0x955d || c->pc == 0x9560 || c->pc == 0x9563 || c->pc == 0x9566) {
  ...
  } else if(c->pc == 0x9579 || c->pc == 0x957c || c->pc == 0x957f || c->pc == 0x9581 || c->pc == 0x9583) {
    done = scene_entry_9579_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc >= 0x9599 && c->pc <= 0x95a8) {
    done = scene_cleanup_9599_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc == 0x9251 || c->pc == 0x9254 || c->pc == 0x9256 || c->pc == 0x9259 || c->pc == 0x925c || c->pc == 0x925e || c->pc == 0x9261 || c->pc == 0x9263 || (c->pc >= 0x9265 && c->pc <= 0x9299)) {
    done = scene_entry_9251_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc >= 0x929c && c->pc <= 0x92a1) {
    done = scene_callee_929c_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc >= 0x92a4 && c->pc <= 0x92bd) {
    done = scene_alt_92a4_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc == 0x92bf) {
    done = scene_read_92bf_step(c, c->pc); x->display_control_steps += done;
  ...
  } else if(c->pc >= 0x99ad && c->pc <= 0x99c1) {
    done = scene_post_99ad_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc == 0x9af5) {
  ...
  } else if(c->pc == 0xfce2 || c->pc == 0xfce6) {
    done = scene_wrap_fce2_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc == 0x9ae3 || c->pc == 0x9ae7) {
    done = scene_wrap_9ae3_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc == 0x9aff || c->pc == 0x9b03) {
    done = scene_wrap_9aff_step(c, c->pc); x->display_control_steps += done;
  } else if(c->pc >= 0x9aec && c->pc <= 0x9af4) {
    done = scene_wrap_9aec_step(c, c->pc); x->display_control_steps += done;
