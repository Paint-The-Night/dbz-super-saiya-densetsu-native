...
static const int stateVersion = 0x445a0001; // DBZ local format: includes DSP output history
/*
1: initial version
...
// Candidate headers can point into arbitrary ROM data. Invalid exponents must
// not trigger undefined shifts while scoring those candidates.
static uint32_t headerSize(uint8_t exponent) {
  return exponent <= 21 ? UINT32_C(0x400) << exponent : 0;
}
...
static void readHeader(const uint8_t* data, int length, int location, CartHeader* header) {
  ...
  header->romSize = headerSize(data[location + 0x17]);
  header->ramSize = headerSize(data[location + 0x18]);
  ...
  if(header->maker == 0x33) {
    ...
    header->gameCode[4] = 0;
    header->flashSize = headerSize(data[location - 4]);
    header->exRamSize = headerSize(data[location - 3]);
