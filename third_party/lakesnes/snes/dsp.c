...
void dsp_handleState(Dsp* dsp, StateHandler* sh) {
  ...
  // Output history affects exact resampling after a checkpoint restore.
  sh_handleWords(sh, &dsp->sampleOffset, NULL);
  for(int i = 0; i < 0x800; i++) sh_handleWordsS(sh, &dsp->sampleBuffer[i], NULL);
}
...
static void dsp_decodeBrr(Dsp* dsp, int ch) {
  ...
  for(int i = 0; i < 4; i++) {
    ...
    if(s > 7) s -= 16;
    if(shift <= 0xc) {
      // Signed left shifts are undefined for negative BRR samples. Multiply
      // within the bounded nibble range, then explicitly round toward -inf.
      s *= 1 << shift;
      s = s >= 0 ? s / 2 : -((-s + 1) / 2);
    } else {
      s = s < 0 ? -4096 : 0;
