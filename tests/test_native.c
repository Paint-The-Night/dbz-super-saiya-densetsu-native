...
int main(int argc,char **argv) {
  ...
  {
    const uint16_t sites[] = {0x8a0e,0x8a10,0x8a11,0x8a14,0x8a15,0x8a16,0x8a19,0x8a1a,0x8a1e,0x8a21};
    for(unsigned n=0;n<10;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0);
      ca.k=cb.k=1; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1ff;
      a->ram[0x0d59]=b->ram[0x0d59]=0x12; a->ram[0x1e3]=b->ram[0x1e3]=0x00; a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 8a0e actor index"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
    const uint16_t sites[] = {0xc8ab,0xc8ad,0xc8af,0xc8b1,0xc8b2,0xc8b4,0xc8b6,0xc8b7,
                              0xc8b8,0xc8bb,0xc8bd,0xc8bf,0xc8c1,0xc8c3,0xc8c7};
    for(unsigned n=0;n<15;n++) {
      ...
      require(dbz_native_step(&ca,stats),"expected c8ab actor table"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
  ...
  }
  {
    ...
    for(unsigned n=0;n<10;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0);
      ca.k=cb.k=1; ca.xf=cb.xf=false; ca.mf=cb.mf=(n==0 || n>=8); ca.sp=cb.sp=0x1ff;
      a->ram[0x1a2]=b->ram[0x1a2]=0x20; a->ram[0x1a4]=b->ram[0x1a4]=0x40; a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 863a actor pointer"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
  ...
  {
    const uint16_t sites[] = {0x9535,0x9538,0x953a,0x953c,0x953e,0x9540,0x9543,0x9544,0x9546,0x954c,0x954e,0x9550,0x9554,0x9556,0x9559,0x955d,0x9560,0x9563,0x9566};
    for(unsigned n=0;n<19;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
  ...
  {
    const uint16_t sites[] = {0x957c,0x957f,0x9581};
    for(unsigned n=0;n<3;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true;
      a->ram[0x2140]=b->ram[0x2140]=(n==1 ? 4 : 0); ca.a=cb.a=(n==2 ? 4 : ca.a); a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 957c scene setup"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
    const uint16_t sites[] = {0x9583};
    for(unsigned n=0;n<1;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1ff; a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 9583 scene continuation"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
    const uint16_t sites[] = {0x9599,0x959b,0x959d,0x959f,0x95a2,0x95a4,0x95a6,0x95a8};
    for(unsigned n=0;n<8;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1ff; a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 9599 cleanup"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  { Cpu ca=make_cpu(a,0x9251,0), cb=make_cpu(b,0x9251,0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; a->ram[0x01a1]=b->ram[0x01a1]=0x80; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 9251 scene entry"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  {
    const uint16_t sites[] = {0x9254,0x9256};
    for(unsigned n=0;n<2;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; a->count=b->count=0;
      if(n==0) ca.n=cb.n=true;
      require(dbz_native_step(&ca,stats),"expected 9251 scene gate"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
    const uint16_t sites[] = {0x9259,0x925c,0x925e,0x9261,0x9263};
    for(unsigned n=0;n<5;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; a->ram[0x01e7]=b->ram[0x01e7]=0; a->ram[0x01a0]=b->ram[0x01a0]=0x0a; ca.a=cb.a=(n==4?0x0a:ca.a); a->count=b->count=0;
      require(dbz_native_step(&ca,stats),"expected 9259 scene path"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
  }
  {
    const uint16_t sites[] = {0x9265,0x9268,0x926a,0x926c,0x926e,0x9270,0x9272,0x9274,0x9276};
    for(unsigned n=0;n<9;n++) {
      memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
      Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.a=cb.a=(n==1?0x0f:(n==3?0x08:(n==5?0x1b:(n==7?0x0d:0)))); a->ram[0x01a0]=b->ram[0x01a0]=ca.a; a->count=b->count=0;
  ...
  { const uint16_t sites[] = {0x929c,0x929f,0x92a1}; for(unsigned n=0;n<3;n++) { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.a=cb.a=(n==1?0x80:0); a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 929c callee gate"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); } }
  { const uint16_t sites[] = {0x92a4,0x92a6,0x92a9,0x92ac,0x92ae,0x92af,0x92b2,0x92b7,0x92ba,0x92bd}; for(unsigned n=0;n<10;n++) { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,sites[n],0), cb=make_cpu(b,sites[n],0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.a=cb.a=(n==9?0x80:0); a->ram[0x01bd]=b->ram[0x01bd]=2; a->ram[0x1648]=b->ram[0x1648]=0; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 92a4 alternate path"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); } }
  ...
  { const uint16_t pcs[] = {0x9756,0x9758,0x975a,0x975c,0x975e,0x9760}; for(unsigned i=0;i<sizeof(pcs)/sizeof(pcs[0]);i++) { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,pcs[i],0), cb=make_cpu(b,pcs[i],0); ca.k=cb.k=3; ca.db=cb.db=0; ca.dp=cb.dp=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.a=cb.a=0x1234; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected actor helper instruction"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); } }
  { const uint16_t pcs[] = {0x9761,0x9763,0x9764,0x9765}; for(unsigned i=0;i<4;i++) { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,pcs[i],0), cb=make_cpu(b,pcs[i],0); ca.k=cb.k=3; ca.db=cb.db=0; ca.dp=cb.dp=0; ca.xf=cb.xf=false; ca.mf=cb.mf=(pcs[i]==0x9761); ca.a=cb.a=0x1234; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected actor width instruction"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); } }
  { const uint16_t pcs[] = {0x9767,0x976b,0x976d,0x976e,0x9770,0x9771,0x9775,0x9777,0x9778}; for(unsigned i=0;i<sizeof(pcs)/sizeof(pcs[0]);i++) { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,pcs[i],0), cb=make_cpu(b,pcs[i],0); ca.k=cb.k=3; ca.db=cb.db=0; ca.dp=cb.dp=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.a=cb.a=0x1234; ca.x=cb.x=0x12; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected actor data instruction"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); } }
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x977a,0), cb=make_cpu(b,0x977a,0); ca.k=cb.k=3; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1fd; a->ram[0x1fe]=b->ram[0x1fe]=0x78; a->ram[0x1ff]=b->ram[0x1ff]=0x56; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected actor PLX"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x977b,0), cb=make_cpu(b,0x977b,0); ca.k=cb.k=3; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1fc; a->ram[0x1fd]=b->ram[0x1fd]=0x34; a->ram[0x1fe]=b->ram[0x1fe]=0x12; a->ram[0x1ff]=b->ram[0x1ff]=0x02; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected actor RTL"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  ...
  /* $05D260: ($CAFA,92) B6B2 → STA #$80 at $14D7/$0020,Y; ORA $B8; JMP $CA8D A=0 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd260,0), cb=make_cpu(b,0xd260,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd272); word(b,0x1fe,0xd272);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x14c2]=b->ram[0x14c2]=0x00; /* B6B2 → Y=$1400 */
    a->ram[0xb8]=b->ram[0xb8]=0x03;
    a->ram[0x1420]=b->ram[0x1420]=0x11;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d260");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d260 JMP CA8D A=0");
    require(a->ram[0x14d7]==0x80 && a->ram[0x1420]==0x80,"d260 wrote #$80");
    require(a->ram[0xb8]==0x83,"d260 ORA into B8");
  }
  /* $05D1E1: ($CAFA,9A) B7!=#$4D → LDA #1 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1e1,0), cb=make_cpu(b,0xd1e1,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1f2); word(b,0x1fe,0xd1f2);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x10;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d1e1 reject");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01,"d1e1 reject CA8D");
  }
  /* $05D1E1: B7==#$4D and $15F1 BMI → JMP $D4E7 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1e1,0), cb=make_cpu(b,0xd1e1,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1f2); word(b,0x1fe,0xd1f2);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x4d;
    a->ram[0x15f1]=b->ram[0x15f1]=0x80;
    unsigned steps=0;
    while(!(ca.pc==0xd4e7 && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d1e1 accept");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xd4e7,"d1e1 JMP D4E7");
  }
  /* $05D1F4: ($CAFA,96) B7!=#$29 → join reject → PLY / LDA #1 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1f4,0), cb=make_cpu(b,0xd1f4,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ca.y=cb.y=0x1400;
    word(a,0x1fe,0xd25e); word(b,0x1fe,0xd25e);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x00;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d1f4 reject");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01 && ca.y==0x1400,"d1f4 reject");
  }
  /* $05D209: ($CAFA,94) B7 not in face set → reject CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd209,0), cb=make_cpu(b,0xd209,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ca.y=cb.y=0x1234;
    word(a,0x1fe,0xd25e); word(b,0x1fe,0xd25e);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x11;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d209 reject");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01 && ca.y==0x1234,"d209 reject");
  }
  /* $05D209: B7=#$24, $001B==6, B582 Z-clear, $0034==1 → JMP $D4E7 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd209,0), cb=make_cpu(b,0xd209,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    word(a,0x1fe,0xd25e); word(b,0x1fe,0xd25e);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x24;
    a->ram[0x14c2]=b->ram[0x14c2]=0x00; /* B6B2→$1400; B6C0→$1440 */
    a->ram[0x141b]=b->ram[0x141b]=0x06;
    a->ram[0x1460]=b->ram[0x1460]=0x05; /* type → B582 X=5 */
    a->ram[0x8205]=b->ram[0x8205]=0x00; /* bit7 clear → Z clear after EOR #$80 */
    a->ram[0x1474]=b->ram[0x1474]=0x01; /* $0034 at Y=$1440 accept */
    unsigned steps=0;
    while(!(ca.pc==0xd4e7 && ca.k==5) && steps++<80) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d209 accept");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xd4e7 && ca.y==0x00aa,"d209 accept D4E7 Y restored");
  }


  /* ==== CAFA D1D6 / D05A / D06D / D095 / D038 / D118 / D0BA ==== */

  /* $05D1D6: ($CAFA,BE) $071B nonzero → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1d6,0), cb=make_cpu(b,0xd1d6,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x071b]=b->ram[0x071b]=0x01;
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d1d6 rts");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"d1d6 RTS");
  }
  /* $05D1D6: $071B==0 → LDA #0 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1d6,0), cb=make_cpu(b,0xd1d6,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1df); word(b,0x1fe,0xd1df);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x071b]=b->ram[0x071b]=0x00;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d1d6 ca8d");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d1d6 JMP CA8D");
  }
  /* $05D05A: ($CAFA,B0) B0BE/B0F9 then #$80 seeds / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd05a,0), cb=make_cpu(b,0xd05a,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd06b); word(b,0x1fe,0xd06b);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x1401]=b->ram[0x1401]=0x2a;
    a->ram[0xb7]=b->ram[0xb7]=0x00;
    a->ram[0x01d6]=b->ram[0x01d6]=0x00;
    ...
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<80) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d05a");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d05a JMP CA8D");
    require(a->ram[0x1480]==0x80 && a->ram[0x14a0]==0x80,"d05a #$80 seeds");
  }
  /* $05D06D: ($CAFA,A8) dual-slot seed / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd06d,0), cb=make_cpu(b,0xd06d,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd093); word(b,0x1fe,0xd093);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x1480]=b->ram[0x1480]=0x55;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d06d");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d06d JMP CA8D");
    require(a->ram[0x14a0]==0x55 && a->ram[0x1481]==0x80 && a->ram[0x14a1]==0x80,"d06d face");
    require(getword(a,0x1484)==0x0080 && getword(a,0x14a4)==0x0080,"d06d X");
    require(getword(a,0x1486)==0x0028 && getword(a,0x14a6)==0x0090,"d06d Y");
  }
  /* $05D095: ($CAFA,A2) m16 coord copy / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd095,0), cb=make_cpu(b,0xd095,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd0b8); word(b,0x1fe,0xd0b8);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    word(a,0x1424,0x00aa); word(b,0x1424,0x00aa);
    word(a,0x1426,0x00c0); word(b,0x1426,0x00c0);
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d095");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d095 JMP CA8D");
    require(getword(a,0x1484)==0x00aa && getword(a,0x1486)==0x0080,"d095 coords");
    require(a->ram[0x1481]==0x85 && a->ram[0x14a0]==0x80,"d095 face");
  }
  /* $05D038: ($CAFA,AE) pinch below #$C8 → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd038,0), cb=make_cpu(b,0xd038,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    word(a,0x1426,0x0033); word(b,0x1426,0x0033);
    a->ram[0x1484]=b->ram[0x1484]=0x40;
    a->ram[0x14a4]=b->ram[0x14a4]=0x50;
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d038 rts");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"d038 RTS");
    require(getword(a,0x1486)==0x0033 && getword(a,0x14a6)==0x0033,"d038 Y copy");
    require(a->ram[0x1484]==0x3e && a->ram[0x14a4]==0x52,"d038 pinch");
  }
  /* $05D038: $14A4 reaches #$C8 → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd038,0), cb=make_cpu(b,0xd038,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd057); word(b,0x1fe,0xd057);
    ca.sp=cb.sp=0x1fd;
    word(a,0x1426,0x0011); word(b,0x1426,0x0011);
    a->ram[0x1484]=b->ram[0x1484]=0x10;
    a->ram[0x14a4]=b->ram[0x14a4]=0xc6; /* +2 → #$C8 */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d038 ca8d");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d038 JMP CA8D");
    require(a->ram[0x14a4]==0xc8,"d038 hit C8");
  }
  /* $05D118: ($CAFA,CA) through JSL $0593A9 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd118,0), cb=make_cpu(b,0xd118,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd13c); word(b,0x1fe,0xd13c);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1401]=b->ram[0x1401]=0x85; /* B572 &7F → 5 */
    a->ram[0x8205]=b->ram[0x8205]=0x00; /* !=9 → no INC $04 */
    a->ram[0x1402]=b->ram[0x1402]=0x10; /* CMP $04=#$2F → BCC → Y=$0137 */
    unsigned steps=0;
    while(!(ca.pc==0x93a9 && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d118");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x93a9 && ca.k==5 && ca.y==0x0137,"d118 JSL 93A9 Y=137");
    require(a->ram[0x04]==0x2f,"d118 DP+04");
  }
  /* $05D0BA: ($CAFA,A0) BMI copy path → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd0ba,0), cb=make_cpu(b,0xd0ba,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1480]=b->ram[0x1480]=0x80; /* BMI → copy */
    for(unsigned i=0;i<8;i++) a->ram[0x1420+i]=b->ram[0x1420+i]=(uint8_t)(0x10+i);
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<120) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d0ba copy");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"d0ba copy RTS");
    require(a->ram[0x1480]==0x10 && a->ram[0x14a0]==0x10,"d0ba copied");
    require(a->ram[0x1481]==0x91 && a->ram[0x14a1]==0x91,"d0ba ORA #$80");
    require(a->ram[0x1484]==0x7e && a->ram[0x14a4]==0x82,"d0ba mid");
    require(getword(a,0x1486)==0x005f && getword(a,0x14a6)==0x005f,"d0ba Y");
  }
  /* $05D0BA: BPL pinch path ($1484!=#$50) → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd0ba,0), cb=make_cpu(b,0xd0ba,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1480]=b->ram[0x1480]=0x00; /* BPL */
    a->ram[0x1484]=b->ram[0x1484]=0x54;
    a->ram[0x14a4]=b->ram[0x14a4]=0x60;
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d0ba pinch");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"d0ba pinch RTS");
    require(a->ram[0x1484]==0x52 && a->ram[0x14a4]==0x62,"d0ba pinch");
  }
  /* $05D0BA: BPL finish ($1484==#$50) → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd0ba,0), cb=make_cpu(b,0xd0ba,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd116); word(b,0x1fe,0xd116);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1480]=b->ram[0x1480]=0x00;
    a->ram[0x1484]=b->ram[0x1484]=0x50;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d0ba finish");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"d0ba finish CA8D");
    require(a->ram[0x1429]==0x8f && a->ram[0x1489]==0x8f && a->ram[0x14a9]==0x8f,"d0ba #$8F");
  }



  /* ==== CAFA CF08 / CF0F / CF60 / CF65 / CFB1 / CFB9 ==== */

  /* $05CFB1: STZ $1434 / LDA #0 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcfb1,0), cb=make_cpu(b,0xcfb1,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcfb7); word(b,0x1fe,0xcfb7);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x1434]=b->ram[0x1434]=0x55;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb1");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00 && a->ram[0x1434]==0x00,"cfb1 CA8D");
  }
  /* $05CFB9: mismatch copy path → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcfb9,0), cb=make_cpu(b,0xcfb9,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1420]=b->ram[0x1420]=0x11;
    a->ram[0x14a0]=b->ram[0x14a0]=0x22; /* mismatch → copy */
    for(unsigned i=0;i<8;i++) a->ram[0x1420+i]=b->ram[0x1420+i]=(uint8_t)(0x30+i);
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<120) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb9 copy");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"cfb9 copy RTS");
    require(a->ram[0x1420]==0x30 && a->ram[0x1460]==0x30 && a->ram[0x1480]==0x30 && a->ram[0x14a0]==0x30,"cfb9 copied");
    require(a->ram[0x1481]==0x00 && a->ram[0x14a1]==0x00,"cfb9 cleared faces");
    require(getword(a,0x1486)==0x0100 && getword(a,0x14a6)==0x0100,"cfb9 Y=$100");
  }
  /* $05CFB9: match pinch ($1464 < #$98) → RTS via helper */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcfb9,0), cb=make_cpu(b,0xcfb9,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    /* helper source bytes at $1420..; keep $1420==$14A0 for match path */
    for(unsigned i=0;i<6;i++) a->ram[0x1420+i]=b->ram[0x1420+i]=(uint8_t)(0xa0+i);
    a->ram[0x1420]=b->ram[0x1420]=0x44;
    a->ram[0x14a0]=b->ram[0x14a0]=0x44; /* match */
    a->ram[0x1421]=b->ram[0x1421]=0x5a; /* face for ORA #$80 */
    a->ram[0x1424]=b->ram[0x1424]=0x50;
    a->ram[0x1464]=b->ram[0x1464]=0x60; /* +2 → 0x62 < 0x98 */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<200) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb9 pinch");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"cfb9 pinch RTS");
    require(a->ram[0x1424]==0x4e && a->ram[0x1464]==0x62,"cfb9 pinch coords");
    require(a->ram[0x1481]==0xda,"cfb9 pinch ORA face"); /* 0x5a|0x80 */
  }
  /* $05CFB9: match finish ($1464 reaches #$98) → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcfb9,0), cb=make_cpu(b,0xcfb9,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd026); word(b,0x1fe,0xd026);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1420]=b->ram[0x1420]=0x44;
    a->ram[0x14a0]=b->ram[0x14a0]=0x44;
    a->ram[0x1424]=b->ram[0x1424]=0x10;
    a->ram[0x1464]=b->ram[0x1464]=0x96; /* +2 → 0x98 */
    a->ram[0x1461]=b->ram[0x1461]=0x21;
    for(unsigned i=0;i<6;i++) a->ram[0x1440+i]=b->ram[0x1440+i]=(uint8_t)(0xb0+i);
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<200) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb9 finish");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"cfb9 finish CA8D");
    require(a->ram[0x1464]==0x98 && a->ram[0x14a1]==0xa1,"cfb9 finish face"); /* 0x21|0x80 */
  }
  /* $05CF08: direct seed path through $CF6F/$C07B → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf08,0), cb=make_cpu(b,0xcf08,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcf5e); word(b,0x1fe,0xcf5e);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0xb7]=b->ram[0xb7]=0x00; /* not 60 → Y=#$000C at CF6F */
    a->ram[0x14e8]=b->ram[0x14e8]=0x02; /* CF6F path needs this if entered via CF0F; CF08 skips */
    /* face bytes at $1421.. for copy loop; LDA dp,X X=$1401 offset $20 → $1421 */
    for(unsigned i=0;i<7;i++) a->ram[0x1421+i]=b->ram[0x1421+i]=(uint8_t)(0x10+i);
    a->ram[0x1421]=b->ram[0x1421]=0x01; /* &7F != 3 → add path */
    word(a,0x1424,0x0010); word(b,0x1424,0x0010);
    word(a,0x1426,0x0020); word(b,0x1426,0x0020);
    /* C07B needs enough state; it is native — plant minimal */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<400) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf08");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"cf08 CA8D");
    require(a->ram[0x1480]==0x09,"cf08 face seed");
  }
  /* $05CF0F: $14E8!=2 → early JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf0f,0), cb=make_cpu(b,0xcf0f,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcf5e); word(b,0x1fe,0xcf5e);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x14e8]=b->ram[0x14e8]=0x00;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf0f early");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"cf0f early CA8D");
  }
  /* $05CF6F: shared helper add-path (face!=3) → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf6f,0), cb=make_cpu(b,0xcf6f,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0xb7]=b->ram[0xb7]=0x11; /* !=60 → Y=#$000C */
    a->ram[0x1421]=b->ram[0x1421]=0x01; /* !=3 */
    word(a,0x1424,0x0010); word(b,0x1424,0x0010);
    word(a,0x1426,0x0020); word(b,0x1426,0x0020);
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf6f add");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"cf6f add RTS");
    require(getword(a,0x1484)==0x001c && getword(a,0x1486)==0x0020,"cf6f add coords"); /* 10+C */
  }
  /* $05CF6F: face==3 path → ADC #$000C on Y */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf6f,0), cb=make_cpu(b,0xcf6f,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0xb7]=b->ram[0xb7]=0x60; /* Y=#$FFE8 */
    a->ram[0x1421]=b->ram[0x1421]=0x03;
    word(a,0x1424,0x0010); word(b,0x1424,0x0010);
    word(a,0x1426,0x0020); word(b,0x1426,0x0020);
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf6f face3");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"cf6f face3 RTS");
    require(getword(a,0x1484)==0x0010 && getword(a,0x1486)==0x002c,"cf6f face3 coords");
  }
  /* $05CF60: stop at JSR $D743 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf60,0), cb=make_cpu(b,0xcf60,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcfb0); word(b,0x1fe,0xcfb0);
    ca.sp=cb.sp=0x1fd;
    unsigned steps=0;
    while(!(ca.pc==0xd743 && ca.k==5) && steps++<5) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf60");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xd743 && ca.k==5,"cf60 reached D743");
  }
  /* $05CF65: stop at JSR $D6E2 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf65,0), cb=make_cpu(b,0xcf65,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcfb0); word(b,0x1fe,0xcfb0);
    ca.sp=cb.sp=0x1fd;
    unsigned steps=0;
    while(!(ca.pc==0xd6e2 && ca.k==5) && steps++<5) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf65");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xd6e2 && ca.k==5,"cf65 reached D6E2");
  }
  /* $05CF68: $14E8!=2 → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcf68,0), cb=make_cpu(b,0xcf68,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x14e8]=b->ram[0x14e8]=0x01;
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cf68 early");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"cf68 early RTS");
  }

  /* ==== CAFA D13E ($CAFA,9E) ==== */

  /* $05D13E: entry through JSL $0593A9 with Y from DP+$04 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd13e,0), cb=make_cpu(b,0xd13e,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    word(a,0x04,0x0012); word(b,0x04,0x0012);
    unsigned steps=0;
    while(!(ca.pc==0x93a9 && ca.k==5) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e entry");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x93a9 && ca.k==5 && ca.y==0x0012,"d13e JSL 93A9 Y=12");
  }
  /* $05D144: B7 not 62/67, Y!=BD/CA → LDA #2 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd144,0), cb=make_cpu(b,0xd144,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x00;
    ...
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e fallthrough");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x02,"d13e fallthrough CA8D A=2");
    require(getword(a,0x0726)==0x0000,"d13e no 0726 write");
  }
  /* $05D144: B7==#$62 → STY #$FFFF at $0726, then fallthrough CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd144,0), cb=make_cpu(b,0xd144,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x62;
    word(a,0x04,0x0001); word(b,0x04,0x0001);
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e b7=62");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x02,"d13e b7=62 CA8D");
    require(getword(a,0x0726)==0xffff,"d13e 0726=FFFF");
  }
  /* $05D144: B7==#$67 → same $0726 seed */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd144,0), cb=make_cpu(b,0xd144,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x67;
    word(a,0x04,0x0002); word(b,0x04,0x0002);
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e b7=67");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && getword(a,0x0726)==0xffff,"d13e b7=67 0726");
  }
  /* $05D144: Y==#$BD → stop at JSR $934B */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd144,0), cb=make_cpu(b,0xd144,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x00;
    word(a,0x04,0x00bd); word(b,0x04,0x00bd);
    unsigned steps=0;
    while(!(ca.pc==0x934b && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e Y=BD");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x934b && ca.k==5,"d13e BD reached 934B");
  }
  /* $05D15E: after $934B — long,X → DP+$00; BRA join → JSL $1C8044 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd15e,0), cb=make_cpu(b,0xd15e,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ca.x=cb.x=0x0002; /* $059366+2 → #$02 */
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    unsigned steps=0;
    while(!(ca.pc==0x8044 && ca.k==0x1c) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e BD join");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x8044 && ca.k==0x1c,"d13e BD reached 1C8044");
    require(a->ram[0x00]==0x02 && a->ram[0x01]==0x00 && a->ram[0x02]==0x00,"d13e BD DP seeds");
  }
  /* $05D144: Y==#$CA → stop at JSR $934B */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd144,0), cb=make_cpu(b,0xd144,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    a->ram[0xb7]=b->ram[0xb7]=0x00;
    word(a,0x04,0x00ca); word(b,0x04,0x00ca);
    unsigned steps=0;
    while(!(ca.pc==0x934b && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e Y=CA");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x934b && ca.k==5,"d13e CA reached 934B");
  }
  /* $05D172: CA path seed through first JSL $1C806C */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd172,0), cb=make_cpu(b,0xd172,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ca.x=cb.x=0x0001; /* $05936B+1 → #$14 */
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    word(a,0x0b0c,0x55aa); word(b,0x0b0c,0x55aa);
    unsigned steps=0;
    while(!(ca.pc==0x806c && ca.k==0x1c) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e CA 806C");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x806c && ca.k==0x1c,"d13e CA reached 1C806C");
    require(getword(a,0x17ed)==0x55aa,"d13e CA 17ED from 0B0C");
    require(a->ram[0x17ea]==0x14 && a->ram[0x17eb]==0x00 && a->ram[0x17ec]==0x00,"d13e CA 17EA seeds");
  }
  /* $05D189: post-806C remap through JSL $1C8136 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd189,0), cb=make_cpu(b,0xd189,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    word(a,0x17ef,0x1111); word(b,0x17ef,0x1111);
    word(a,0x17f1,0x2222); word(b,0x17f1,0x2222);
    a->ram[0x17f3]=b->ram[0x17f3]=0x33;
    unsigned steps=0;
    while(!(ca.pc==0x8136 && ca.k==0x1c) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e CA 8136");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x8136 && ca.k==0x1c,"d13e CA reached 1C8136");
    require(getword(a,0x17ea)==0x1111 && getword(a,0x17ec)==0x2222,"d13e CA remap EA/EC");
    require(a->ram[0x17ee]==0x33 && getword(a,0x17ef)==0x0064,"d13e CA EE/EF");
    require(a->ram[0x17f1]==0x00 && a->ram[0x17f2]==0x00 && a->ram[0x17f3]==0x00,"d13e CA cleared F1-F3");
  }
  /* $05D1AE: pair-SBC $0B05 − $17EA then JSL $1C8044 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xd1ae,0), cb=make_cpu(b,0xd1ae,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xd1d4); word(b,0x1fe,0xd1d4);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x0b05]=b->ram[0x0b05]=0x50;
    a->ram[0x0b06]=b->ram[0x0b06]=0x02;
    a->ram[0x17ea]=b->ram[0x17ea]=0x10;
    a->ram[0x17eb]=b->ram[0x17eb]=0x01;
    unsigned steps=0;
    while(!(ca.pc==0x8044 && ca.k==0x1c) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected d13e SBC join");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0x8044 && ca.k==0x1c,"d13e SBC reached 1C8044");
    require(a->ram[0x0b05]==0x40 && a->ram[0x0b06]==0x01,"d13e SBC result");
    require(a->ram[0x00]==0x10 && a->ram[0x01]==0x01 && a->ram[0x02]==0x00,"d13e SBC DP seeds");
  }
  ...
  /* $05CDE1: store DP+$05 → $1509,X; LDA #2 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcde1,0), cb=make_cpu(b,0xcde1,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcdec); word(b,0x1fe,0xcdec);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x06;
    a->ram[0x05]=b->ram[0x05]=0x3c;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cde1");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x02 && a->ram[0x1509+6]==0x3c,"cde1 CA8D");
  }
  /* $05CDF7: DEC×2 then nonzero → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcdf7,0), cb=make_cpu(b,0xcdf7,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04;
    a->ram[0x05]=b->ram[0x05]=0x10;
    a->ram[0x150d+4]=b->ram[0x150d+4]=0x14; /* -2 → 0x12; &FE=0x12; CMP 0x10 → RTS */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cdf7 RTS");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && a->ram[0x150d+4]==0x12 && a->ram[0x06]==0x02,"cdf7 RTS");
  }
  /* $05CDEE: seed DP+$05=0/$06=1; DEC to match → JMP $CA8D with A=$06 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcdee,0), cb=make_cpu(b,0xcdee,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xce11); word(b,0x1fe,0xce11);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x08;
    /* After CDEE: $05=0, $06=1. Counter: want (counter-2)&FE == $05 (==0) */
    a->ram[0x150d+8]=b->ram[0x150d+8]=0x02; /* -2 → 0; &FE=0; BEQ */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cdee CA8D");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01,"cdee CA8D A=1");
    require(a->ram[0x05]==0x00 && a->ram[0x06]==0x01 && a->ram[0x150d+8]==0x00,"cdee seeds");
  }

  /* ==== CAFA CE13 / CE60 / CE8B / CEFF ==== */

  /* $05CEFF: LDA#0/XBA/LDA $04/AND#$7F/TAX/RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xceff,0), cb=make_cpu(b,0xceff,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x96; /* &7F → 0x16 */
    ca.a=cb.a=0x55aa;
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ceff");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && ca.x==0x0016,"ceff TAX");
  }
  /* $05CE13: DP+$05!=0, counter below threshold → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce13,0), cb=make_cpu(b,0xce13,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x02; /* X=2 after CEFF */
    a->ram[0x05]=b->ram[0x05]=0x05; /* nonzero → skip CE4C */
    word(a,0x150d+2,0x0001); word(b,0x150d+2,0x0001); /* INC → 2; CMP 5 → BCC RTS */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce13 early RTS");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && getword(a,0x150d+2)==0x0002,"ce13 early RTS");
  }
  /* $05CE13: DP+$05==0 → seed #$94, CE4C, then expire → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce13,0), cb=make_cpu(b,0xce13,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xce4a); word(b,0x1fe,0xce4a);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04; /* X=4 */
    a->ram[0x05]=b->ram[0x05]=0x00; /* trigger seed #$94 + CE4C */
    word(a,0x4f,0x0002); word(b,0x4f,0x0002); /* AND #2 → 2; ASL→4; DEC→3; DEC→2 */
    word(a,0x150b+4,0x0010); word(b,0x150b+4,0x0010);
    word(a,0x150d+4,0x0093); word(b,0x150d+4,0x0093); /* INC→94 == threshold → expire */
    a->ram[0x1507+4]=b->ram[0x1507+4]=0x11;
    a->ram[0x1560+2]=b->ram[0x1560+2]=0x22; /* X>>1 = 2 after LSR */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<80) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce13 expire");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x02,"ce13 expire CA8D");
    require(a->ram[0x05]==0x94,"ce13 seeded 94");
    require(getword(a,0x150b+4)==0x0012,"ce13 CE4C delta"); /* 10+2 */
    require(a->ram[0x1507+4]==0x00 && a->ram[0x1560+2]==0x00,"ce13 cleared slots");
  }
  /* $05CE4C: helper alone → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce4c,0), cb=make_cpu(b,0xce4c,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    ca.x=cb.x=0x0006;
    word(a,0x4f,0x0000); word(b,0x4f,0x0000); /* → ASL/DEC/DEC = -2 = $FFFE */
    word(a,0x150b+6,0x0005); word(b,0x150b+6,0x0005);
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce4c");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && getword(a,0x150b+6)==0x0003,"ce4c +(-2)");
  }
  /* $05CE60: $1507,Y < 5 → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce60,0), cb=make_cpu(b,0xce60,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x08;
    a->ram[0x1507+8]=b->ram[0x1507+8]=0x04; /* <5 */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce60 early");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff,"ce60 early RTS");
  }
  /* $05CE60: BMI clear path with bit-mask table → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce60,0), cb=make_cpu(b,0xce60,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xce85); word(b,0x1fe,0xce85);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x80; /* X=0x00 after AND 7F from CEFF... 0x80&7F=0 */
    /* Use index 2 via $04=2, and BMI via high bit on a different path:
     * CE6C loads DP+$04 again for BPL — same $04. Need BMI so $04 bit7 set.
     * Conflict: CEFF uses $04&7F for X, then CE6C uses $04 for BPL.
     * So $04=$82 → CEFF X=2; CE6C BMI taken. */
    a->ram[0x04]=b->ram[0x04]=0x82;
    a->ram[0x1507+2]=b->ram[0x1507+2]=0x06; /* >=5 */
    a->ram[0x150a+2]=b->ram[0x150a+2]=0x01; /* mask index 1 → $CE87=$FE */
    a->ram[0xb0]=b->ram[0xb0]=0xff;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce60 mask");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01,"ce60 CA8D");
    require(a->ram[0x1507+2]==0x00,"ce60 cleared 1507");
    require(a->ram[0xb0]==0xfe,"ce60 masked B0"); /* FF & FE */
  }
  /* $05CE60: BPL skip mask → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce60,0), cb=make_cpu(b,0xce60,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xce85); word(b,0x1fe,0xce85);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x0a; /* X=0x0A; BPL */
    a->ram[0x1507+0x0a]=b->ram[0x1507+0x0a]=0x08;
    a->ram[0xb0]=b->ram[0xb0]=0xaa;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce60 bpl");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && a->ram[0xb0]==0xaa && a->ram[0x1507+0x0a]==0x08,"ce60 bpl untouched");
  }
  /* $05CE8B: stop at JSR $B572 after face seed */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce8b,0), cb=make_cpu(b,0xce8b,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcefd); word(b,0x1fe,0xcefd);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04;
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x1401]=b->ram[0x1401]=0x12; /* type for B572 */
    unsigned steps=0;
    while(!(ca.pc==0xb572 && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ce8b to B572");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xb572 && ca.k==5 && a->ram[0x1507+4]==0x01,"ce8b reached B572");
    require(a->ram[0x00]==0x00,"ce8b STZ dp0");
  }
  /* $05CE8B: continue through B572 (type!=9) to B6B2 then JMP $DD51 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xce8b,0), cb=make_cpu(b,0xce8b,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    ...
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04;
    a->ram[0x05]=b->ram[0x05]=0x07;
    a->ram[0x06]=b->ram[0x06]=0x00;
    word(a,0x20,0x1400); word(b,0x20,0x1400); /* != $1440 → LDA #$40 */
    a->ram[0x1401]=b->ram[0x1401]=0x01; /* type → B572 result unlikely 9 */
    a->ram[0xb7]=b->ram[0xb7]=0x00; /* !=3A skip EOR */
    /* B572 reads long table — plant identity-ish: ensure path completes.
     * Run until DD51; B572/B6B2 are native. */
    unsigned steps=0;
    while(!(ca.pc==0xdd51 && ca.k==5) && steps++<200) {
      a->count=b->count=0;
      if(!dbz_native_step(&ca,stats)) {
        /* allow interpreter only if somehow outside — should not happen */
        require(false,"ce8b full left native");
      }
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xdd51 && (ca.a&0xff)==0x03,"ce8b DD51");
    require(a->ram[0x1507+4]==0x01 && a->ram[0x150a+4]==0x07,"ce8b slots");
    require(a->ram[0x150f+4]==0x40,"ce8b face flag");
    require(getword(a,0x150b+4)==0 && getword(a,0x150d+4)==0,"ce8b cleared words");
  }



  /* ==== CAFA CD02 / CD81 / CD96 / CDB6 / CDCA ==== */

  /* $05CD96: seed $150D/$150B; ensure $1507; LDA #4 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcd96,0), cb=make_cpu(b,0xcd96,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcdb4); word(b,0x1fe,0xcdb4);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x06;
    a->ram[0x05]=b->ram[0x05]=0x11;
    word(a,0x06,0x1234); word(b,0x06,0x1234);
    a->ram[0x1507+6]=b->ram[0x1507+6]=0x00; /* seed #$05 */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cd96");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x04,"cd96 CA8D");
    require(a->ram[0x150d+6]==0x11 && getword(a,0x150b+6)==0x1234,"cd96 slots");
    require(a->ram[0x1507+6]==0x05,"cd96 seeded 1507");
  }
  /* $05CD81: after CE4C/D566 carry clear → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcd81,0), cb=make_cpu(b,0xcd81,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04; /* X=4 */
    a->ram[0x05]=b->ram[0x05]=0x07; /* copied to DP+$04 before D566 */
    word(a,0x4f,0x0000); word(b,0x4f,0x0000);
    word(a,0x150b+4,0x0010); word(b,0x150b+4,0x0010);
    word(a,0x20,0x1400); word(b,0x20,0x1400); /* D566 LDY DP+$20 */
    a->ram[0x142b]=b->ram[0x142b]=0x01; /* INC→2; CMP DP+$04(=7) → C clear → RTS */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<60) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cd81 RTS");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && a->ram[0x142b]==0x02,"cd81 early RTS");
  }
  /* $05CD81: D566 carry set → JMP $DD51 A=2 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcd81,0), cb=make_cpu(b,0xcd81,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcd94); word(b,0x1fe,0xcd94);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x02;
    a->ram[0x05]=b->ram[0x05]=0x03;
    word(a,0x4f,0x0000); word(b,0x4f,0x0000);
    word(a,0x150b+2,0x0000); word(b,0x150b+2,0x0000);
    word(a,0x20,0x1400); word(b,0x20,0x1400);
    a->ram[0x142b]=b->ram[0x142b]=0x03; /* INC→4 >= 3 → C set */
    unsigned steps=0;
    while(!(ca.pc==0xdd51 && ca.k==5) && steps++<80) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cd81 DD51");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xdd51 && (ca.a&0xff)==0x02,"cd81 DD51");
  }
  /* $05CDB6: snapshot $1540/$1546/$1502 → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcdb6,0), cb=make_cpu(b,0xcdb6,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcddf); word(b,0x1fe,0xcddf);
    ca.sp=cb.sp=0x1fd;
    word(a,0x1540,0xabcd); word(b,0x1540,0xabcd);
    a->ram[0x1546]=b->ram[0x1546]=0x55;
    a->ram[0x1502]=b->ram[0x1502]=0x66;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cdb6");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x00,"cdb6 CA8D");
    require(getword(a,0x1542)==0xabcd && a->ram[0x1544]==0x55 && a->ram[0x1545]==0x66,"cdb6 snap");
  }
  /* $05CDCA: restore snapshot → JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcdca,0), cb=make_cpu(b,0xcdca,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcddf); word(b,0x1fe,0xcddf);
    ca.sp=cb.sp=0x1fd;
    word(a,0x1542,0x1111); word(b,0x1542,0x1111);
    a->ram[0x1544]=b->ram[0x1544]=0x22;
    a->ram[0x1545]=b->ram[0x1545]=0x33;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cdca");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && getword(a,0x1540)==0x1111,"cdca restore");
    require(a->ram[0x1546]==0x22 && a->ram[0x1502]==0x33,"cdca bytes");
  }
  /* $05CD02: $150C!=#$81 → stop at JSR $DFF3 after scale index */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcd02,0), cb=make_cpu(b,0xcd02,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcd7f); word(b,0x1fe,0xcd7f);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x02; /* X=2 after CEFF */
    a->ram[0x05]=b->ram[0x05]=0x03; /* ASL×4 → X=0x30 for DFF3 */
    a->ram[0x150c+2]=b->ram[0x150c+2]=0x00; /* !=81 skip seed */
    unsigned steps=0;
    while(!(ca.pc==0xdff3 && ca.k==5) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cd02 DFF3");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xdff3 && ca.x==0x0030,"cd02 reached DFF3");
  }

  /* ==== CAFA CC04 / CC0E / CC25 / CC92 / CCA0 / CCBB / CCC8 / CCEE ==== */

  /* $05CC04: DP+$04 → $1506; LDA #1 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcc04,0), cb=make_cpu(b,0xcc04,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcc0c); word(b,0x1fe,0xcc0c);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x9a;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<10) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cc04");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01 && a->ram[0x1506]==0x9a,"cc04");
  }
  /* $05CC0E: equal after LSR → LDA #1 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcc0e,0), cb=make_cpu(b,0xcc0e,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcc20); word(b,0x1fe,0xcc20);
    ca.sp=cb.sp=0x1fd;
    word(a,0x0b0c,0x0004); word(b,0x0b0c,0x0004); /* LSR → 2 */
    word(a,0x0b05,0x0002); word(b,0x0b05,0x0002); /* equal → CA8D */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cc0e CA8D");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x01,"cc0e equal");
  }
  /* $05CC0E: after LSR greater → JMP $D4E7 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcc0e,0), cb=make_cpu(b,0xcc0e,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcc23); word(b,0x1fe,0xcc23);
    ca.sp=cb.sp=0x1fd;
    word(a,0x0b0c,0x0008); word(b,0x0b0c,0x0008); /* LSR → 4 */
    word(a,0x0b05,0x0002); word(b,0x0b05,0x0002); /* 4>2 → BCS D4E7 */
    unsigned steps=0;
    while(!(ca.pc==0xd4e7 && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cc0e D4E7");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xd4e7,"cc0e D4E7");
  }
  /* $05CCBB: STA DP+$05 → $1511,X; LDA #2 / JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xccbb,0), cb=make_cpu(b,0xccbb,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xccc6); word(b,0x1fe,0xccc6);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x08;
    a->ram[0x05]=b->ram[0x05]=0x44;
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ccbb");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && (ca.a&0xff)==0x02 && a->ram[0x1511+8]==0x44,"ccbb");
  }
  /* $05CCA0: $150F nonzero → keep #$FFB0; JMP $CA8D */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcca0,0), cb=make_cpu(b,0xcca0,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xccb9); word(b,0x1fe,0xccb9);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x04;
    a->ram[0x150f+4]=b->ram[0x150f+4]=0x01; /* nonzero → Y stays FFB0 */
    unsigned steps=0;
    while(!(ca.pc==0xca8d && ca.k==5) && steps++<30) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cca0");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xca8d && getword(a,0x150b+4)==0xffb0,"cca0 FFB0");
  }
  /* $05CC92: SBC #$04 then fall into $CCC8 → stop at JSR $DFF3 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcc92,0), cb=make_cpu(b,0xcc92,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcd01); word(b,0x1fe,0xcd01);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x02;
    a->ram[0x05]=b->ram[0x05]=0x01; /* ASL×4 → X=0x10 */
    a->ram[0x150d+2]=b->ram[0x150d+2]=0x14; /* -4 → 0x10 */
    a->ram[0x150c+2]=b->ram[0x150c+2]=0x00; /* !=81 */
    unsigned steps=0;
    while(!(ca.pc==0xdff3 && ca.k==5) && steps++<50) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cc92 DFF3");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xdff3 && a->ram[0x150d+2]==0x10 && ca.x==0x0010,"cc92 DFF3");
  }
  /* $05CC25: no button bits → skip stream; $14CD below threshold → RTS */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcc25,0), cb=make_cpu(b,0xcc25,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcafe); word(b,0x1fe,0xcafe);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x47]=b->ram[0x47]=0x00;
    a->ram[0x48]=b->ram[0x48]=0x00;
    word(a,0x14cd,0x0005); word(b,0x14cd,0x0005); /* INC→6 < 0x12C → RTS */
    unsigned steps=0;
    while(!(ca.pc==0xcaff) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cc25 RTS");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcaff && a->ram[0x153b]==0x82 && getword(a,0x14cd)==0x0006,"cc25 RTS");
  }
  /* $05CCC8: skip seed → stop at JSR $DFF3 (same prelude as $CD02) */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xccc8,0), cb=make_cpu(b,0xccc8,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fe,0xcd01); word(b,0x1fe,0xcd01);
    ca.sp=cb.sp=0x1fd;
    a->ram[0x04]=b->ram[0x04]=0x02;
    a->ram[0x05]=b->ram[0x05]=0x02; /* ASL×4 → X=0x20 */
    a->ram[0x150c+2]=b->ram[0x150c+2]=0x00; /* !=81 */
    unsigned steps=0;
    while(!(ca.pc==0xdff3 && ca.k==5) && steps++<50) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ccc8 DFF3");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xdff3 && ca.x==0x0020,"ccc8 DFF3");
  }
  /* $05CCEE mid-entry: ASL×4 / JSR $DFF3 / PLX / EOR #$40 / BRA $CD38 */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xccee,0), cb=make_cpu(b,0xccee,0);
    ca.k=cb.k=5; ca.db=cb.db=0; ca.xf=cb.xf=false;
    word(a,0x1fc,0x0004); word(b,0x1fc,0x0004); /* PHX saved X */
    word(a,0x1fe,0xcd7f); word(b,0x1fe,0xcd7f);
    ca.sp=cb.sp=0x1fb;
    ca.a=cb.a=0x0003; /* → ASL×4 = 0x30 for DFF3 */
    a->ram[0x150f+4]=b->ram[0x150f+4]=0x00;
    unsigned steps=0;
    while(!(ca.pc==0xcd38 && ca.k==5) && steps++<120) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected ccee to CD38");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcd38 && ca.x==0x0004 && (ca.a&0xff)==0x40,"ccee EOR join CD38");
  }

  /* $01CFB2: dialogue fetch leaf — first script byte ($F4) falls through to $CFC4.
   * i18n filter is identity under JA (default); bus/CPU must match lakesnes. */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xcfb2,0), cb=make_cpu(b,0xcfb2,0);
    ca.k=cb.k=1; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true;
    ca.y=cb.y=0;
    word(a,0x00,0x82dc); word(b,0x00,0x82dc);
    a->ram[0x02]=b->ram[0x02]=0x06; /* long ptr 06:82DC (main script string 0) */
    word(a,0x20,0x0010); word(b,0x20,0x0010);
    word(a,0x1fe,0xcfc4); word(b,0x1fe,0xcfc4); /* unused; leaf does not RTL */
    ca.sp=cb.sp=0x1ff;
    unsigned steps=0;
    while(!(ca.pc==0xcfc4) && steps++<20) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb2 text fetch");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcfc4 && ca.y==0x0001,"cfb2 advanced to CFC4");
    require(a->ram[0x04]==0xf4,"cfb2 stored script byte");
    require((uint8_t)ca.a==0xf4,"cfb2 A holds F4");
  }
  /* $01CFB2 space path: byte $01 → INX / JSL $01D00B / BRA $CFB4 then exit on next */
  {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    /* Plant a tiny stream in bank-6 ROM mirror via ram overlay is unavailable;
     * use WRAM long pointer into bank 0 RAM-mirrored lowmem by pointing at
     * 00:0200 where we plant F4 after a leading 01. Banks 0 ROM only at $8000+;
     * low WRAM works for bank 0 addresses < $8000. */
    Cpu ca=make_cpu(a,0xcfb2,0), cb=make_cpu(b,0xcfb2,0);
    ca.k=cb.k=1; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true;
    ca.y=cb.y=0; ca.x=cb.x=0;
    a->ram[0x300]=b->ram[0x300]=0x01;
    a->ram[0x301]=b->ram[0x301]=0xf4;
    word(a,0x00,0x0300); word(b,0x00,0x0300);
    a->ram[0x02]=b->ram[0x02]=0x00;
    word(a,0x20,0x0004); word(b,0x20,0x0004);
    /* D00B RTL frame */
    word(a,0x1fd,0x0000); /* placeholder; jsl pushes */
    ca.sp=cb.sp=0x1ff;
    unsigned steps=0;
    while(!(ca.pc==0xcfc4) && steps++<40) {
      a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected cfb2 space+fetch");
      lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    }
    require(ca.pc==0xcfc4,"cfb2 space path reached CFC4");
