...
int main(int argc,char **argv) {
  ...
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x8ae5,0), cb=make_cpu(b,0x8ae5,0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 8ae5 STZ"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x8ae8,0), cb=make_cpu(b,0x8ae8,0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 8ae8 STZ"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x8aeb,0), cb=make_cpu(b,0x8aeb,0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1ff; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 8aeb JSL"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
  { memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram)); Cpu ca=make_cpu(a,0x8aef,0), cb=make_cpu(b,0x8aef,0); ca.k=cb.k=0; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=true; ca.sp=cb.sp=0x1fc; a->ram[0x1fd]=b->ram[0x1fd]=0xee; a->ram[0x1fe]=b->ram[0x1fe]=0x8a; a->ram[0x1ff]=b->ram[0x1ff]=0x00; a->count=b->count=0; require(dbz_native_step(&ca,stats),"expected 8aef RTL"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b); }
