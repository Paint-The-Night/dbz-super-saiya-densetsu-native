...
int main(int argc,char **argv) {
  ...
  /* $06EE42: PHP entry prefix. */
  for(unsigned variant=0; variant<32; variant++) {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xee42,variant), cb=make_cpu(b,0xee42,variant);
    ca.k=cb.k=6; ca.sp=cb.sp=0x1ff; word(a,0x200,0x8fff); word(b,0x200,0x8fff);
  ...
  for(unsigned variant=0; variant<8; variant++) {
    memset(a->ram,0,sizeof(a->ram)); memset(b->ram,0,sizeof(b->ram));
    Cpu ca=make_cpu(a,0xee46,variant), cb=make_cpu(b,0xee46,variant);
    ca.k=cb.k=6; ca.db=cb.db=0; ca.xf=cb.xf=false; ca.mf=cb.mf=false; ca.sp=cb.sp=0x1ff;
    word(a,0x200,0x8fff); word(b,0x200,0x8fff); a->count=b->count=0;
    require(dbz_native_step(&ca,stats),"expected ee46 JSL"); lakesnes_cpu_runOpcode(&cb); compare(&ca,&cb,a,b);
    require(ca.k==6 && ca.pc==0xf089 && ca.sp==0x1fc,"ee46 F089 boundary");
  }
